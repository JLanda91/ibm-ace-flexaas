...
has_project_types = ACEConnection.get(uri="/apiv2",
                                      expected_rc=200,
                                      parse_json=True,
                                      auth=auth,
                                      params=None,
...
for project_type in filter(lambda x: has_project_types[x], project_types):
    projects = ACEConnection.get(uri=f"/apiv2/{project_type}",
                                 expected_rc=200,
                                 parse_json=True,
                                 auth=auth,
                                 params=None,
    ...
    for project_name, project_uri in projects:
        flows = ACEConnection.get(uri=f"{project_uri}/messageflows",
                                  expected_rc=200,
                                  parse_json=True,
                                  auth=auth,
                                  params=None,
        ...
        for flow_name, flow_uri in flows:
            recording_on = ACEConnection.get(uri=f"{flow_uri}",
                                             expected_rc=200,
                                             parse_json=True,
                                             auth=auth,
                                             params=None,
            ...
            if not recording_on:
                recording_toggle = ACEConnection.post(uri=f"{flow_uri}/start-recording",
                                                      data="",
                                                      expected_rc=200,
                                                      parse_json=False,
                                                      auth=auth)
